...
export const confirmEmail = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        if (decoded.purpose !== 'confirm-registration') {
            return next(new AppError('Невалиден токен за потвърждение', 400));
        }
...
export const requestLoginLink = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        res.status(200).json({
            success: true,
            message: 'Изпратен е имейл с линк за вход'
        });
    } catch (error) {
        return next(new AppError('Възникна грешка при изпращането на имейла. Моля, опитайте отново по-късно!', 500));
    }
...
export const verifyEmailLogin = catchAsync(async (req, res, next) => {
    ...
    if (!token) {
        return next(new AppError('Не е предоставен токен', 400));
    }
